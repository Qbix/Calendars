...
function Calendars_after_Assets_credits_spend($params)
{
    ...
    $intent = Q::ifset($params, 'options', 'intent', null);
    $referrerUserId = $intent ? $intent->getInstruction('referrerUserId') : null;
    ...
    $paymentsInfo = Assets_Credits::getPaymentsInfo($fromUserId, array(
        'publisherId' => $toPublisherId,
        'streamName'  => $toStreamName
    ), null, $referrerUserId);
    if ($paymentsInfo["conclusion"]["fullyPaid"]) {
    ...
    Calendars_Event::going($stream, $fromUserId, $going, array(
        'skipPayment' => true // avoid infinite recursion
    ));
}